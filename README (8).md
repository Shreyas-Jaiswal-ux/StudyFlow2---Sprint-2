...
```javascript
// editingTaskId is null when adding, holds the task ID when editing
...
function handleSubmit(e) {
  ...
  if (editingTaskId) {
    updateTask(editingTaskId, title, deadline, priority, module); // update path
    cancelEditing(); // resets editingTaskId to null and restores form
  } else {
    addTask(title, deadline, priority, module); // create path
...
Using `(dayOfWeek + 6) % 7` correctly maps JavaScript's Sunday-zero convention to a Monday-zero index without any conditional branching. The result is a `YYYY-MM-DD` string pair that can be compared directly against deadline strings.
...
let filterState = {
  ...
  module: ''         // free-text search string
};

function getFilteredTasks() {
  let result = [...tasks]; // never mutate the original array
...
| NFR-02 | Mobile-first responsive | Yes | 360-428px base styles, 768px media query breakpoint, T-19 |
| NFR-03 | Load within 3 seconds | Yes | No external dependencies; app loads from local files with zero network requests |
| NFR-04 | Minimalist design with calm colour palette | Yes | Blues and neutrals throughout; high-contrast colours reserved for priority indicators only, T-22 |
| NFR-05 | Minimum 16px body text | Yes | CSS base font-size 16px; all body text uses `var(--font-family)` at 15-16px minimum, T-19 |
| NFR-06 | Minimum 44x44px tap targets | Yes | All buttons and inputs use `min-height: 44px` in CSS, verified in T-21 |
| NFR-07 | Input validation | Yes | Empty title, missing deadline, past date blocked, T-04 to T-07 |
